 ...
 * for Game Coordinator services, e.g. {@link DotaAccountProfileCardCallback}
 */

public class DotaClientGetProfileCardAsync implements Runnable {

    private static final Logger LOGGER = LoggerFactory.getLogger(DotaClientGetProfileCardAsync.class);
    ...
    public DotaClientGetProfileCardAsync(String username, String password) {
    ...
    public static void main(String[] args) {
        ...
        new DotaClientGetProfileCardAsync(args[0], args[1]).run();
    }
    ...
    public void run() {
        ...
        manager.subscribe(DotaAccountProfileCardCallback.class, this::onAccountProfile);
    ...
    private void onClientWelcome(ClientWelcomeCallback callback) {
        ...
        dotaClientAsync.requestAccountProfileCard(ACCOUNT_ID);
    }

    private void onAccountProfile(DotaAccountProfileCardCallback callback) {
        try {
            System.out.println("AccountProfile: " + System.lineSeparator() + mapper.writeValueAsString(callback));
        } catch (JsonProcessingException e) {
            System.out.println(e.getMessage());
        }
        steamUser.logOff();
